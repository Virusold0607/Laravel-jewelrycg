                    ...
                    @foreach ($arrProductMaterials[$material->id] as $product_material)
                        <tr class="pm{{$product_material->diamond_id}}_pmt{{$product_material->material_type_id}}_m{{$material->id}}">
                            ...
                            @if($material->id == 1)
                                ...
                                <td><input type="number" name="diamond_amount[]" class="form-control"
                                           value="{{ $product_material->diamond_amount }}"/></td>
                                <td>
                                        ...
                                        @if (isset($arrDiamondTypeClarity) && count($arrDiamondTypeClarity) > 0)
                                            @foreach ($arrDiamondTypeClarity as $clarity)
                                                @if(isset($diamondPrices[$product_material->diamond_id]) && count($diamondPrices[$product_material->diamond_id]) > 0)
                                                    <option value="{{$clarity->id}}" {{$clarity->id == $diamondPrices[$product_material->diamond_id]['clarity'] ? "selected" : ""}}>{{$clarity->name}}
                                                        ({{$clarity->letters}})
                                                    </option>
                                                @else
                                                    <option value="{{$clarity->id}}">{{$clarity->name}}
                                                        ({{$clarity->letters}})
                                                    </option>
                                                @endif
                                            @endforeach
                                    ...
                                    <select class="form-control" name="diamond_color[]">
                                        @if (isset($arrDiamondTypeColors) && count($arrDiamondTypeColors) > 0)
                                            @foreach ($arrDiamondTypeColors as $color)
                                                @if(isset($diamondPrices[$product_material->diamond_id]) && count($diamondPrices[$product_material->diamond_id]) > 0)
                                                    <option value="{{$color->id}}" {{$color->id == $diamondPrices[$product_material->diamond_id]['color'] ? "selected" : ""}}>{{$color->name}}
                                                        ({{$color->letters}})
                                                    </option>
                                                @else
                                                    <option value="{{$color->id}}">{{$color->name}}({{$color->letters}}
                                                        )
                                                    </option>
                                                @endif
                                            @endforeach
                                ...
                                @if(isset($diamondPrices[$product_material->diamond_id]) && count($diamondPrices[$product_material->diamond_id]) > 0)
                                    <td><input type="text" name="lab_price[]" class="form-control"
                                               value="{{ $diamondPrices[$product_material->diamond_id]['lab_price'] }}"/>
                                    </td>
                                @else
                                    <td><input type="text" name="lab_price[]" class="form-control" value=""/></td>
                                @endif
                                @if(isset($diamondPrices[$product_material->diamond_id]) && count($diamondPrices[$product_material->diamond_id]) > 0)
                                    <td><input type="text" name="natural_price[]" class="form-control"
                                               value="{{ $diamondPrices[$product_material->diamond_id]['natural_price'] }}"/>
                                    </td>
                                @else
                                    <td><input type="text" name="natural_price[]" class="form-control" value=""/></td>
                                @endif
                                <input type="hidden" name="material_weight[]" class="form-control"
                                       value="{{ $product_material->material_weight }}"/>
                                <input type="hidden" class="form-control" id="diamond_id" name="diamond_id[]"
                                       value="{{ $product_material->diamond_id }}"/>
                                <input type="hidden" class="form-control" name="diamond_mmsize[]"
                                       value="{{ $product_material->mm_size }} mm"/>
                                <input type="hidden" class="form-control" name="material_typename[]"
                                       value="{{ $product_material->material_type_name }}"/>
                            @else
                                <input type="hidden" name="diamond_id[]">
                                <input type="hidden" name="diamond_amount[]" class="form-control"
                                       value="{{ $product_material->diamond_amount }}"/>
                                <td><input type="number" name="material_weight[]" class="form-control"
                            ...
                            <td class='text-center action'>
                                ...
                                <input type="hidden" class="form-control" id="material_id" name="material_id[]"
                                       value="{{ $material->id }}"/>
                                <input type="hidden" class="form-control" id="is_diamond" name="is_diamond[]"
                                       value="{{ $material->id == 1 ? 1: 0 }}"/>
